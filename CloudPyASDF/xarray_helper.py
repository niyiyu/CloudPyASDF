...
def embed_xarray(ds, group, name = "XR"):
    ...
    xr_obj = {'data': list(ds.data_vars) + list(ds.coords),
            'attrs': dict(ds.attrs)}
    ...
    for _obj, _val in xr_obj.items():
        if isinstance(_val, list):
            ...
            for _v in _val:
                ...
                _attr = ds[_v].attrs
                _attr.update({'_shape': ds[_v].data.shape,
                          '_type': "coords" if _v in list(ds.coords) else "variable"})
                xroot[_v].create_dataset('attrs', 
                            data = np.fromstring(str(_attr), dtype = 'int8'))
